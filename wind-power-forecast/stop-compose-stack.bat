...
docker compose -p wind-power-deployment -f "%DEPLOY_COMPOSE%" stop minio pgadmin
...
docker ps -a --format "table {{.Names}}\t{{.Status}}" | findstr /I "wind-power-kingbase wind-power-minio wind-power-pgadmin"
exit /b 0
