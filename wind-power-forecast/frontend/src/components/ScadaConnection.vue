<template>
  <div class="scada-connection-page page-shell">
    ...
    <div v-loading="loading" class="connection-grid">
      ...
      <div v-for="conn in connections" :key="conn.id" class="connection-card" :class="`status-${conn.status}`">
        <div class="card-top">
          <div class="card-status">
            <span class="status-dot" :class="conn.status"></span>
        ...
        <div class="card-body">
          ...
          <div v-if="conn.last_data_at" class="last-data">
            最后数据: {{ formatTime(conn.last_data_at) }}
          </div>
          <div v-if="conn.status_message" class="status-msg" :class="{ 'is-error': conn.status === 'error' }">
            {{ conn.status_message }}
          </div>
        </div>
        ...
        <div class="card-actions">
          ...
          <el-button-group>
            <el-button size="small" @click="openEditDialog(conn)" :disabled="conn.status === 'running'">编辑</el-button>
            <el-button size="small" type="danger" @click="handleDelete(conn)" :disabled="conn.status === 'running'">删除</el-button>
          </el-button-group>
        </div>
      </div>
    </div>
      ...
      <el-form :model="form" :rules="formRules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="连接协议" prop="protocol">
          <el-radio-group v-model="form.protocol">
            <el-radio value="http_poll">HTTP轮询（模拟/开发）</el-radio>
...
export default {
  ...
  setup() {
    ...
    const openAddDialog = () => {
      ...
      Object.assign(form, {
        farm_code: '', name: '', protocol: 'http_poll',
