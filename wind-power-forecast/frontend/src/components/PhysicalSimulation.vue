...
import {
  ...
  getSimulationTurbines,
  getSimulationReadings
} from '../api/physicalSimulationApi';
...
export default {
  ...
  methods: {
    async fetchFarmNames() {
      ...
      try {
        const response = await getSimulationConditions();
    ...
    async handleFarmSelection(farmName) {
      ...
      try {
        const [conditionsRes, turbinesRes] = await Promise.all([
          getSimulationConditions({ farm_name: farmName }),
          getSimulationTurbines({ farm_name: farmName }),
        ]);
    ...
    async runSimulation() {
      ...
      try {
        ...
        const readingPromises = uniqueConditions.map(conditionId => 
          getSimulationReadings({ condition_id: conditionId })
        );
