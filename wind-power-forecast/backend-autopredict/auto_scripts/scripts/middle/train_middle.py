...
def train_and_evaluate(X_train, y_train, X_val, y_val, params_list, scaler, model_folder_today, val_timestamps=None, feature_names=None, save_importance=True):
    ...
    for params in params_list:
        ...
        if val_timestamps is not None:
            ...
            if val_timestamps is not None:
                ...
                try:
                    ...
                    print(f"时间加权评估结果 - 加权RMSE: {weighted_rmse:.4f}, 加权K: {weighted_k:.4f}")
                    logger.info(f"时间加权评估结果 - 加权RMSE: {weighted_rmse:.4f}, 加权K: {weighted_k:.4f}")
        ...
        # 生成详细的评估结果日志
        log_msg = f"{params['name']} 评估结果 - RMSE: {rmse:.4f}, K: {k:.4f}, 加权评分: {weighted_score:.4f}"
        if weighted_rmse is not None and weighted_k is not None:
            log_msg += f", 时间加权RMSE: {weighted_rmse:.4f}, 时间加权K: {weighted_k:.4f}"
...
def train_multiple_datasets(data, months_list, train_ratio, lags, window_size, model_folder_today):
    ...
    for months in months_list:
        ...
        for params in all_params:
            ...
            # --- BEGIN MODIFICATION ---
            logger.info(f"即将为模型 {model_name} (数据源: {months_desc}) 开始训练。")
...
def apply_feature_selection(X_train, X_val, flat_feature_names, algo_type, top_n=3000, current_date=None):
    ...
    baseline_stats_path = '../../config/feature_scaling_parameters_middle.json'
