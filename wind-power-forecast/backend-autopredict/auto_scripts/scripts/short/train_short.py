...
import json  # 新增：用于加载基准统计配置文件
import datetime  # 用于自适应决策系统的时间戳
import shutil  # 用于文件备份操作
from config_short import Today, FEATURE_IMPORTANCE_DIR
from utils_short import calculate_daily_averaged_k, evaluate_with_time_weights
from data_processor_short import (
    preprocess_data, filter_data_by_date, feature_engineering,
    scale_data, create_time_window_pre,
    create_flattened_windows_with_indices
)
from models_short import get_unified_params
import psutil
    ...
    for params in params_list:
        ...
        if val_timestamps is not None:
            ...
            if val_timestamps is not None:
                ...
                try:
                    weighted_score, _, _, weighted_rmse, weighted_k = evaluate_with_time_weights(y_val, y_pred, val_timestamps)
        ...
        if weighted_score is None:
            ...
            weighted_rmse = rmse  # 使用标准RMSE作为加权RMSE
            weighted_k = k       # 使用标准K作为加权K
        ...
        print(f"  时间加权结果 - 加权RMSE: {weighted_rmse:.4f}, 加权K: {weighted_k:.4f}")
        logger.info(f"{params['name']} 评估结果 - RMSE: {rmse:.4f}, K: {k:.4f}, 加权评分: {weighted_score:.4f}")
        logger.info(f"  时间加权结果 - 加权RMSE: {weighted_rmse:.4f}, 加权K: {weighted_k:.4f}")
...
def load_baseline_stats(filepath: str) -> dict:
    ...
    try:
        ...
        # 适配新的JSON格式（包含metadata和feature_stats）
...
def calculate_overall_distribution_shift_score(current_stats: dict, baseline_stats: dict) -> float:
    ...
    Args:
        current_stats: 当前数据的统计信息 {feature: {'mean': value, 'std': value}}
        baseline_stats: 基准数据的统计信息 {feature: {'mean': value, 'std': value}}
    
    Returns:
        float: 总体分布偏移分数，范围为[0, +inf)，0表示完全一致，值越大表示差异越大
    """
    if not baseline_stats:
...
def update_baseline_stats(current_stats, baseline_stats_path, backup_enabled=True):
    ...
    try:
        # 1. 备份原有文件
        if backup_enabled and os.path.exists(baseline_stats_path):
            backup_path = f"{baseline_stats_path}.backup.{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        config_data['metadata']['last_update_time'] = datetime.datetime.now().isoformat()
...
def apply_feature_selection(X_train, X_val, flat_feature_names, algo_type, top_n=3000, current_date=None):
    ...
    baseline_stats_path = '../../config/feature_scaling_parameters_short.json'
    ...
    # 步骤4: 自适应决策系统 - 计算分布偏移分数并决策
    print("步骤4: 启动自适应决策系统...")
    
    # 步骤4.1: 计算总体偏移分数
    shift_score = calculate_overall_distribution_shift_score(current_stats, baseline_stats)
    
    # 步骤4.2: 获取自适应阈值（从配置文件或使用默认值）
    ADAPTATION_THRESHOLD = 0.15  # 默认阈值：平均偏移小于15%被认为是正常漂移
    if baseline_stats and isinstance(baseline_stats, dict):
        # 尝试从配置文件的metadata中获取阈值
        config_threshold = None
        try:
            # 如果baseline_stats是完整的配置文件结构
            if 'metadata' in baseline_stats:
                config_threshold = baseline_stats['metadata'].get('adaptation_threshold')
            # 如果baseline_stats只是feature_stats部分，尝试加载完整配置
            else:
                with open(baseline_stats_path, 'r', encoding='utf-8') as f:
                    full_config = json.load(f)
                    if 'metadata' in full_config:
                        config_threshold = full_config['metadata'].get('adaptation_threshold')
        except Exception as e:
            print(f"警告: 无法从配置文件读取自适应阈值，使用默认值: {e}")
        
        if config_threshold is not None:
            ADAPTATION_THRESHOLD = float(config_threshold)
            print(f"使用配置文件中的自适应阈值: {ADAPTATION_THRESHOLD}")
        else:
            print(f"使用默认自适应阈值: {ADAPTATION_THRESHOLD}")
    
    # 步骤4.3: 自适应决策逻辑
    if baseline_stats and shift_score < ADAPTATION_THRESHOLD:
        # 适应模式：偏移小，认为是授权场站的正常漂移
        print(f"🔄 决策: 偏移分数 {shift_score:.4f} < 阈值 {ADAPTATION_THRESHOLD:.4f}")
        print("🎯 判定为正常概念漂移，执行适应性更新策略以保证最优性能")
        ...
        # 用当前最新数据更新基准指纹文件
        print("📝 正在更新基准指纹文件...")
        update_success = update_baseline_stats(current_stats, baseline_stats_path)
        if update_success:
            print("✅ 基准指纹更新成功，系统已学习当前数据分布")
        else:
            print("⚠️ 基准指纹更新失败，但不影响当前训练")
        
    else:
        # 惩罚模式：偏移大，认为是环境剧变或迁移
        if baseline_stats:
            print(f"🔒 决策: 偏移分数 {shift_score:.4f} >= 阈值 {ADAPTATION_THRESHOLD:.4f}")
            print("⚡ 检测到显著的数据分布偏移，启动保护性降级机制")
        else:
            print("⚠️ 缺少基准数据，启动保护性降级机制")
        
        # 调用惩罚函数，降级保护
        final_importance = penalize_features_by_distribution_shift(
            initial_importance, current_stats, baseline_stats
        )
        print("🛡️ 特征重要性保护性校正已完成")

    # 步骤5: 根据决策结果应用特征选择
    if baseline_stats:  # 只有在有基准数据时才应用校正结果
        print("步骤5: 根据决策结果重新选择特征...")
        # 根据最终重要性排序
        ...
        if selected_indices:
            ...
            print(f"✅ 自适应特征选择完成！最终选出 {len(selected_indices)} 个特征")
            print("Top 10 特征及其最终重要性得分:")
            for feature, importance in sorted_features[:10]:
                print(f"  - {feature}: {importance:.6f}")
            
            print(f"✅ 特征选择后：训练集形状 {X_train_selected.shape}，验证集形状 {X_val_selected.shape}")
            logger.info(f"✅ 自适应特征选择后：训练集形状 {X_train_selected.shape}，验证集形状 {X_val_selected.shape}")
