...
def _try_load_from_db(farm_code, target_dt):
    """尝试从 ecmwf_meteorological_data 表加载预测输入数据
    ...
    try:
        ...
        if df is not None and not df.empty:
            logging.info(f"DB查询成功: {len(df)} rows for {target_dt.strftime('%Y%m%d')}")
            return df
        return None
    except Exception as e:
        logging.warning(f"DB预测输入加载失败: {e}")
        return None
        ...
        if not os.path.exists(csv_file):
            ...
            if DB_PREDICTION_ENABLED:
                ...
                if db_df is not None and not db_df.empty:
                    ...
                    print(f"✅ 从数据库加载预测输入成功，已保存到: {csv_file}")
                    logging.info(f"从数据库加载预测输入成功，已保存到: {csv_file}")
                else:
                    print(f"数据库中也无数据，等待 {wait_interval} 秒后重试: {csv_file}")
...
def main():
    ...
    if args.mode == 'train' or args.mode == 'all':
        print(f"创建训练监视线程...")
        logging.info(f"创建训练监视线程...")
        train_thread = Thread(target=monitor_training, args=(today_date,), name="TrainThread")
        print(f"启动训练线程...")
        logging.info(f"启动训练线程...")
        train_thread.start()
        if args.mode == 'train': # If only training, wait for it and exit
            print(f"等待训练线程完成...")
            logging.info(f"等待训练线程完成...")
            train_thread.join()
            print_section("训练模式完成")
            logging.info("训练模式完成")
            return # Exit after training if mode is 'train'

    if args.mode == 'predict' or args.mode == 'all':
        # If mode is 'all', training thread is already started if it exists.
        # monitor_prediction internally waits for the model.
        if args.mode == 'all' and train_thread is not None and train_thread.is_alive():
            print(f"模式 'all': 等待训练线程完成才能安全开始预测...")
            logging.info(f"模式 'all': 等待训练线程完成才能安全开始预测...")
            train_thread.join() # Ensure training completes before prediction in 'all' mode

        print(f"创建预测监视线程...")
        logging.info(f"创建预测监视线程...")
        predict_thread = Thread(target=monitor_prediction, args=(today_date,), name="PredictThread")
        print(f"启动预测线程...")
        logging.info(f"启动预测线程...")
        predict_thread.start()
        if args.mode == 'predict': # If only prediction, wait for it and exit
            print(f"等待预测线程完成...")
            logging.info(f"等待预测线程完成...")
            predict_thread.join()
            print_section("预测模式完成")
            logging.info("预测模式完成")
            return # Exit after prediction if mode is 'predict'

    # If mode is 'all', wait for both threads if they were started
    if args.mode == 'all':
        if train_thread is not None and train_thread.is_alive(): # Should be joined already, but as a safeguard
            train_thread.join()
        if predict_thread is not None and predict_thread.is_alive():
            print(f"模式 'all': 等待预测线程完成...")
            logging.info(f"模式 'all': 等待预测线程完成...")
            predict_thread.join()

    print_section("系统任务完成 (基于模式)")
    logging.info("系统任务完成 (基于模式)")
