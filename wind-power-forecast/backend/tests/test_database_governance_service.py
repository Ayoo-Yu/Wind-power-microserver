...
def test_table_classification_distinguishes_governed_families():
    managed = {"users", "training_history", "conditions"}

    assert governance_service.classify_table("users", managed) == "managed"
    assert governance_service.classify_table("training_history", managed) == "legacy_candidate"
    assert governance_service.classify_table("conditions", managed) == "demo_candidate"
...
def test_recommendations_report_legacy_indexes_and_scada_growth(monkeypatch):
    monkeypatch.setenv("NWP_INGESTION_ENABLED", "false")
    migration = {"ready": True}
    tables = [
        {"category": "nwp_partition", "empty_estimate": True},
        {"category": "legacy_candidate", "empty_estimate": True},
    ]
    ...
    assert any("旧链路" in title for title in titles)
