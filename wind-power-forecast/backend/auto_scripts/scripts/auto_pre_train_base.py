...
def _training_period(date_str):
    return str(date_str)[:6]


def get_train_flag_file(date_str):
    period = _training_period(date_str)
    return os.path.join(LOG_DIR, f"{period}_train_done.flag")
...
def _model_folder_today(today_date: str, farm_code: str) -> str:
    ...
    model_period = _training_period(today_date)
    if VARIANT["USE_FARM_CODE"]:
        return os.path.join(MODEL_FOLDER, f'{model_period}_{farm_code}')
    return os.path.join(MODEL_FOLDER, model_period)
...
def _register_models_to_registry(best_models_info, model_folder_today, dataset_path=None):
    ...
    try:
        ...
        wfcapacity = get_farm_capacity(farm_code)
...
def monitor_training(today_date):
    ...
    while True:
        ...
        update_successful = _update_training_csv_from_db(csv_file, farm_code=farm_code)
...
def _try_load_from_db(farm_code, target_dt):
    ...
    try:
        ...
        table = f"ecmwf_grid_{farm_code.lower()}"
        ...
        with db_session() as session:
            ...
            rows = session.execute(
                ...
                {"source": latest_source, "start": start_time, "end": end_time},
            ).fetchall()
        ...
        for ft, lat_g, lon_g, features in rows:
            ...
            for var in variables:
                val = feat.get(var)
                records[ft][f"{var}_{lat_g}_{lon_g}"] = float(val) if val is not None else None

        df = _pd.DataFrame(list(records.values()))
...
def run_main(*, has_farm_code_arg=False):
    ...
    if has_farm_code_arg:
        if not args.farm_code or not args.farm_code.strip():
            print("场站代码不能为空")
            logging.error("场站代码不能为空")
            return
        farm_code = args.farm_code.strip()
    else:
        farm_code = os.environ.get('FARM_CODE', 'DEFAULT_FARM')
    ...
    os.environ['FARM_CODE'] = farm_code
    print(f"设置环境变量 FARM_CODE = {farm_code}")
    logging.info(f"设置环境变量 FARM_CODE = {farm_code}")
