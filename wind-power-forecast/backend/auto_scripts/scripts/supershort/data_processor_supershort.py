...
# Determine the correct path to the merged backend package.
# current_file_dir is ...auto_scripts/scripts/supershort/
current_file_dir = os.path.dirname(os.path.abspath(__file__))
# target_path should be ...backend/
...
if path_to_backend not in sys.path:
    ...
    # print(f"[DEBUG data_processor_supershort] Added to sys.path: {path_to_backend}")
...
def resolve_farm_code(farm_code=None):
    value = farm_code or os.environ.get('FARM_CODE') or os.environ.get('DEFAULT_FARM_CODE') or 'DEFAULT_FARM'
    return str(value).strip()


def load_and_merge_data_from_db(days_to_load: int = None, farm_code=None):
    """
    ...
    farm_code = resolve_farm_code(farm_code)
    logging.info("Starting to load data directly from database for supershort model (using TrainPreShort), farm_code=%s", farm_code)
    ...
    try:
        with db_session() as session:
            ...
            query_actual = session.query(ActualPower.timestamp, ActualPower.wp_true).filter(
...
def get_backfill_value_from_prediction_tables(target_timestamp, session=None, farm_code=None):
    """
    从预测表中获取指定时间戳的回填值
    ...
    try:
        ...
        # SupershortlPower 表现在预计有 wp_pred2 到 wp_pred17 列
        # wp_pred{S} (S from 2 to 17) 在记录时间戳 T_rec 处，是对未来 T_rec + (S-2)*15分钟 的预测
        # 当我们为 target_timestamp 寻找回填值时:
        # 我们需要找到一个 source_timestamp 和一个列 wp_pred{db_col_suffix} (db_col_suffix from 2 to 17)
        # 使得 source_timestamp + (db_col_suffix - 2)*15分钟 = target_timestamp
        # 因此, source_timestamp = target_timestamp - (db_col_suffix - 2)*15分钟
        for db_col_suffix in range(2, 18): # db_col_suffix 从 2 到 17
            # (db_col_suffix - 2) 是实际的预测步长（0步到15步）
            # 例如: db_col_suffix = 2 -> wp_pred2, 对应 (2-2)*15 = 0 分钟偏移 (预测当前记录时间点)
            #        db_col_suffix = 3 -> wp_pred3, 对应 (3-2)*15 = 15 分钟偏移 (预测记录时间点后15分钟)
            #        db_col_suffix = 17 -> wp_pred17, 对应 (17-2)*15 = 225 分钟偏移 (预测记录时间点后225分钟)
            offset_minutes = (db_col_suffix - 2) * 15
            source_timestamp_for_pred = target_timestamp - timedelta(minutes=offset_minutes)
            
            supershort_record = session.query(SupershortlPower).filter(
                SupershortlPower.farm_code == farm_code,
                SupershortlPower.timestamp == source_timestamp_for_pred
            ).first()
            
            if supershort_record:
                # 获取名为 wp_pred{db_col_suffix} 的列的值
                pred_value = getattr(supershort_record, f'wp_pred{db_col_suffix}', None)
                if pred_value is not None and not pd.isna(pred_value):
                    logging.info(f"从SupershortlPower表获取回填值: 源时间戳={source_timestamp_for_pred}, 使用列 wp_pred{db_col_suffix}={pred_value} (目标时间戳={target_timestamp})")
                    if should_close_session and session: # 在找到值并返回前确保关闭会话
                        session.close()
                    return float(pred_value)
        
        # 2. 尝试从ShortlPower表获取值
...
def interpolate_and_ffill_wp_true(df, wp_true_col='wp_true', farm_code=None):
    """
    处理wp_true列中的空值，增强版本：
            ...
            with db_session() as session:
                for idx in result_df.index:
                    if pd.isna(result_df.loc[idx, wp_true_col]):
                        ...
                        backfill_value = get_backfill_value_from_prediction_tables(timestamp, session, farm_code=farm_code)
...
def update_supershort_training_csv_from_db(csv_file_path: str, farm_code=None):
    """
    ...
    farm_code = resolve_farm_code(farm_code)
    logging.info(f"Starting to check and update supershort training CSV: {csv_file_path}, farm_code={farm_code}")
    ...
    try:
        ...
        merged_new_data = interpolate_and_ffill_wp_true(merged_new_data, farm_code=farm_code)
