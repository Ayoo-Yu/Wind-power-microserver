...
def get_supershort_train_flag_file_for_date(date_str):
    period = str(date_str)[:6]
    return os.path.join(log_dir_supershort_train, f"{period}_supershort_train_done.flag")

def get_supershort_train_running_flag_file_for_date(date_str):
    """获取训练进行中的标志文件路径"""
    period = str(date_str)[:6]
    return os.path.join(log_dir_supershort_train, f"{period}_supershort_train_running.flag")

def get_supershort_train_lock_file_for_date(date_str):
    """获取训练文件锁的路径"""
    period = str(date_str)[:6]
    return os.path.join(log_dir_supershort_train, f"{period}_supershort_train.lock")
