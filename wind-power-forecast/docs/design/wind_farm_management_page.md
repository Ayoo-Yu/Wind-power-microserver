...
3. **地理分布 (Map / Scatter)**
   - 基于经纬度显示风电场位置，首版使用 Plotly scatter（已内置于 `PhysicalSimulation`，可复用）。
   - Hover 提示显示场站名称、装机容量、运行状态；点大小按容量动态映射。
   - 若无经纬度数据，自动展示占位提示，不报错。
...
4. 迭代添加地图、可视化模块。（✅ 已完成：散点地图 + 数据准备度堆叠柱状图 + 快捷操作卡片）
5. 更新文档与用户指南。（进行中）

## 实现记录

- **路由与入口**：`/windfarm-management` 新增在 `router/index.js`，侧边栏使用 `view_all_data` 权限控制显示。
- **数据拉取与兜底策略**：
  - 基础数据通过 `useWindFarmStore.loadWindFarms()` 复用 `/report/farms`。
  - 数据准备度优先请求 `/report/farms/data-readiness`；若接口返回 404/405/异常，则回退使用 `windFarms` 中现有的 `training_dataset_count`、`prediction_dataset_count`、`simulation_record_count`。
- **可视化组件**：
  - Plotly 散点图（地理分布）使用 `plotly.js-dist-min`。
  - Chart.js 横向堆叠柱状图（数据准备度）。
  - 支持在 watch 中自动刷新图表，组件卸载时销毁实例避免内存泄漏。
- **快捷操作**：
  - 通过 `setSelectedWindFarm` 同步全局场站，再跳转至相应页面，保证目标页面立即使用最新场站上下文。
- **后续扩展建议**：
  - 若后端补充 `/report/farms/data-readiness`，需根据最终字段调整图表标签/单位。
  - 可在快捷操作卡片上增加权限控制、直接发起任务等行为。

## 验证记录

- 页面导航：侧边菜单进入“风电场管理”后，概览卡片、风场列表、地图、数据准备度图表均正常渲染，无控制台报错。
- 数据准备度：当 `/report/farms/data-readiness` 不存在时自动触发兜底策略，图表仍显示训练/预测/仿真数据条目。
- 快捷操作：选择场站后一键跳转“模型训练 / 功率对比 / 上报配置 / 物理仿真”，各页面加载时已同步被选场站。
- 退回其它页面再进入，图表与地图会重新渲染，未出现 Plotly/Chart.js 重复初始化问题。
