...
后端接口 `/api/v1/system/capabilities` 返回当前部署的能力事实表。前端顶部会提示尚未接入的数据能力。能力状态分为 `available`、`config_required` 和 `unavailable`。
...
| SCADA 实时数据 | production | config_required | 点表、时钟、链路和数据质量验收完成 |
