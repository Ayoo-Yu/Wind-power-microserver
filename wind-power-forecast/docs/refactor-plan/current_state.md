...
- `backend-autopredict/app.py`：为自动预测脚本单独复制出的 Flask 应用，仅提供 PM2 任务管理相关接口，但重新实现了数据库/日志/配置初始化。
...
- **自动预测/调度**：`backend-autopredict/routes/autopredict.py` 大量依赖 PM2 命令行，轮询日志/状态；历史记录由 `TaskHistory` 记录在数据库。
...
- `backend-autopredict/auto_scripts/`：三套（short/middle/supershort）独立的训练与预测脚本树，包含大量 `.py` 和 `.joblib`，通过 PM2 定时执行。
- 调度方式：主后端 `services/scheduler_service.py` 试图初始化 APScheduler；自动预测后端通过 PM2 + flag 文件判断状态。
...
3. **任务调度脆弱**：靠 PM2 命令与 flag 文件追踪状态，缺乏可靠的任务队列和统一日志。
...
- **自动预测路径**：在测试环境模拟 PM2/任务队列，验证任务启动、状态查询、日志抓取流程。
