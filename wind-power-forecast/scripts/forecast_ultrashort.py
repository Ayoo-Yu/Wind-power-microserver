...
def per_shift_dynamic_rolling(
    val_df_raw, test_df_raw,
    val_shift_preds: dict, test_shift_preds: dict,
    cap: float, window_days: int = 14,
) -> list:
    """Per-shift calibration: each of 16 shift models gets its own affine calibrator.
    This matches production — each shift prediction is independently submitted and fixed."""
    y_val = val_df_raw[TARGET].to_numpy(float)
    y_test = test_df_raw[TARGET].to_numpy(float)
    timestamps_val = val_df_raw[TIME_COL].values
    timestamps_test = test_df_raw[TIME_COL].values

    # Build per-shift DataFrames with date column
    def build_shift_records(shift_preds, y_true, timestamps):
        records = []
        for s in range(1, N_SHIFTS + 1):
            ...
            for i in range(len(y_true)):
                if np.isnan(p[i]):
                    continue
                records.append({
                    "date": pd.Timestamp(timestamps[i]).normalize(),
                    TIME_COL: timestamps[i],
                    "actual": y_true[i],
                    f"shift_{s}": p[i],
                })
        return pd.DataFrame(records)

    val_records = build_shift_records(val_shift_preds, y_val, timestamps_val)
    test_records = build_shift_records(test_shift_preds, y_test, timestamps_test)

    # Pivot to get one row per (date, timestamp, actual) with all shift predictions
    shift_cols = [f"shift_{s}" for s in range(1, N_SHIFTS + 1)]

    def pivot_records(records_df):
        # Group by timestamp, aggregate (should be one row per timestamp)
        agg = records_df.groupby([TIME_COL, "date", "actual"]).first().reset_index()
        # Actually we need to pivot the shift columns
        pivoted = records_df.pivot_table(
            index=[TIME_COL, "date", "actual"],
            columns=None,
            values=shift_cols,
            aggfunc="first",
        ).reset_index()
    ...
    # Simpler approach: build a combined DataFrame directly
    def build_combined(shift_preds, y_true, timestamps):
        # Get valid indices (where all shifts have non-NaN predictions)
        # Actually we need indices where at least shift predictions exist after N_SHIFTS
        n = len(y_true)
        dates = pd.Series(timestamps).apply(lambda x: pd.Timestamp(x).normalize()).values
        rows = []
        for i in range(N_SHIFTS, n):
            shifts = {f"shift_{s}": shift_preds[s][i] for s in range(1, N_SHIFTS + 1)}
            if any(np.isnan(v) for v in shifts.values()):
                continue
            row = {"date": dates[i], TIME_COL: timestamps[i], "actual": y_true[i], **shifts}
            rows.append(row)
    ...
    for d in test_dates:
        ...
        # Raw: simple average of all shifts (no calibration)
        p_raw_avg = np.mean(np.column_stack([cur[f"shift_{s}"].to_numpy(float) for s in range(1, N_SHIFTS + 1)]), axis=1)
        ...
        # Method 1: per-shift calibration — fit each shift independently
        p_cal_shifts = np.zeros((len(cur), N_SHIFTS))
        for s in range(1, N_SHIFTS + 1):
            p_fit_s = fit[f"shift_{s}"].to_numpy(float)
            a_s, b_s = fit_affine(y_fit, p_fit_s, cap)
            p_cur_s = cur[f"shift_{s}"].to_numpy(float)
            p_cal_shifts[:, s - 1] = np.clip(a_s * p_cur_s + b_s, 0, cap)
        p_per_shift_avg = np.mean(p_cal_shifts, axis=1)
        day_accs["per_shift_cal"].append(float(1.0 - np.sqrt(np.mean(((y - p_per_shift_avg) / d_floor) ** 2))))

        # Method 2: single calibration on averaged predictions (old approach)
        p_fit_avg = np.mean(np.column_stack([fit[f"shift_{s}"].to_numpy(float) for s in range(1, N_SHIFTS + 1)]), axis=1)
        a_single, b_single = fit_affine(y_fit, p_fit_avg, cap)
        p_single_cal = np.clip(a_single * p_raw_avg + b_single, 0, cap)
        day_accs["single_cal"].append(float(1.0 - np.sqrt(np.mean(((y - p_single_cal) / d_floor) ** 2))))

    rows = []
    for m in ["raw", "per_shift_cal", "single_cal"]:
...
def main():
    ...
    # Calibration
    if should_calibrate:
        print("  Running per-shift dynamic affine calibration...")
        cal_rows = per_shift_dynamic_rolling(
            val_df, test_df, val_shift, test_shift, cap, window_days=14,
        )
        for r in cal_rows:
            if r["method"] == "raw":
                continue
        ...
        best_cal = max([r for r in cal_rows if r["method"] != "raw"], key=lambda r: r["accuracy_percent"])
        if best_cal["accuracy_percent"] > raw["accuracy_percent"]:
            print(f"  Best calibration: {best_cal['method']} — improves accuracy")
        else:
