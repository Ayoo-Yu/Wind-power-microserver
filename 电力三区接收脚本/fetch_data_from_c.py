...
def sync_latest_files():
    ...
    try:
        ...
        batch_date_match = re.match(r"(\d{4})\d{4}", batch_dirs[0][2]) # YYYYMMDD -> YYYYMM
        if not batch_date_match:
            logging.error("无法从批次目录日期部分 {} 提取年月信息。".format(batch_dirs[0][2]))
            return False
        batch_month_day = batch_date_match.group(1) # 应该是 YYYYMM
        local_date_dir = os.path.join(LOCAL_DOWNLOAD_DIR, "{}_{}".format(batch_year, batch_month_day))
        logging.info("将在本地创建或使用日期目录: {}".format(local_date_dir))
        ...
        try:
            ...
            makedirs_exist_ok(LOCAL_TEMP_DIR)
            makedirs_exist_ok(local_date_dir)
        except OSError as e:
        ...
        for file_type, (remote_path, local_final_path) in files_to_download.items():
            local_temp_path = os.path.join(LOCAL_TEMP_DIR, os.path.basename(local_final_path))
            logging.info("开始下载 [{}] 文件: \"{}\" -> \"{}\"".format(file_type, remote_path, local_temp_path))
            ...
            if download_successful:
                ...
                if not os.path.exists(local_temp_path):
                     logging.error("下载声称成功，但本地临时文件不存在: \"{}\"。SFTP 可能内部出错或未实际传输。跳过此文件。".format(local_temp_path))
                     continue
                try:
                    if os.path.getsize(local_temp_path) == 0:
                        logging.warning("下载的文件为空: \"{}\"。可能远程文件为空或下载中断。将删除临时文件。".format(local_temp_path))
                        os.unlink(local_temp_path) # 删除空的临时文件
                        continue # 跳过处理此文件
                except OSError as e:
                    logging.error("检查文件大小失败: \"{}\", 错误: {}. 跳过此文件.".format(local_temp_path, e))
                    continue

                # 移动到最终位置
                try:
                    logging.info("移动文件到最终位置: \"{}\" -> \"{}\"".format(local_temp_path, local_final_path))
                    os.rename(local_temp_path, local_final_path)

                    # 记录状态 (仅在成功移动后)
                    newly_downloaded.add(remote_path)
                    downloaded_count += 1
                    logging.info("文件处理完成: \"{}\"".format(local_final_path))
                except OSError as move_err:
                     logging.error("移动文件失败: {}。文件可能保留在临时目录: \"{}\"".format(move_err, local_temp_path), exc_info=True)
                     # 移动失败，不添加到下载状态，并尝试清理临时文件
                     if os.path.exists(local_temp_path):
                         try: os.unlink(local_temp_path)
                         except OSError: pass
            else:
