...
try:
    ...
    print(f"FATAL ERROR when trying to 'from iec104api import *':") # 修改提示
    print(f"Specific ImportError: {e}") # 打印 e 的具体内容
    print(f"Error type: {type(e)}")
    # 打印 traceback 可以帮助看到错误发生的具体位置
    import traceback
    traceback.print_exc()
    ...
    try:
        ...
        print(f"Files in script directory ({module_dir}): {os.listdir(module_dir)}", file=sys.stderr)
    except Exception as list_e:
        print(f"Could not list script directory contents: {list_e}", file=sys.stderr)
    sys.exit(1)
except Exception as general_ex: # 捕获其他可能的异常，如 SyntaxError 等
    print(f"FATAL ERROR during import setup: {general_ex}")
    print(f"Error type: {type(general_ex)}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
def connect_scada():
    ...
    conn_params.u8OrginatorAddress = 0  # 使用与ctypes定义匹配的拼写
    
    # 初始化文件传输目录路径为空
    conn_params.ai8FileTransferDirPath = b'\x00' * 260  # MAX_DIRECTORY_PATH 通常是260
    
    # 添加详细的结构体字段调试信息
    logger.debug("DEBUG: ---- sConf dump before IEC104LoadConfiguration ----")
    try:
        null_byte = b'\x00'
        source_ip_bytes = bytes(sConf.sClientSet.ai8SourceIPAddress).split(null_byte, 1)[0].decode('ascii', 'ignore')
        logger.debug(f"DEBUG: sConf.sClientSet.ai8SourceIPAddress: {source_ip_bytes}")
    except Exception as e:
        logger.debug(f"DEBUG: Error reading ai8SourceIPAddress: {e}")
    
    logger.debug(f"DEBUG: sConf.sClientSet.benabaleUTCtime: {sConf.sClientSet.benabaleUTCtime}")
    logger.debug(f"DEBUG: sConf.sClientSet.sDebug.u32DebugOptions: {sConf.sClientSet.sDebug.u32DebugOptions}")
    logger.debug(f"DEBUG: sConf.sClientSet.u16TotalNumberofConnection: {sConf.sClientSet.u16TotalNumberofConnection}")
    logger.debug(f"DEBUG: sConf.sClientSet.bAutoGenIEC104DataObjects: {sConf.sClientSet.bAutoGenIEC104DataObjects}")
    logger.debug(f"DEBUG: sConf.sClientSet.u16UpdateBuffersize: {sConf.sClientSet.u16UpdateBuffersize}")
    logger.debug(f"DEBUG: sConf.sClientSet.bClientAcceptTCPconnection: {sConf.sClientSet.bClientAcceptTCPconnection}")
    logger.debug(f"DEBUG: sConf.sClientSet.psClientConParameters is None?: {sConf.sClientSet.psClientConParameters is None}")
    
    try:
        dest_ip_bytes = bytes(conn_params.ai8DestinationIPAddress).split(null_byte, 1)[0].decode('ascii', 'ignore')
        logger.debug(f"DEBUG: conn_params.ai8DestinationIPAddress: {dest_ip_bytes}")
    except Exception as e:
        logger.debug(f"DEBUG: Error reading ai8DestinationIPAddress: {e}")
    
    logger.debug(f"DEBUG: conn_params.u16PortNumber: {conn_params.u16PortNumber}")
    logger.debug(f"DEBUG: conn_params.i16k: {conn_params.i16k}, conn_params.i16w: {conn_params.i16w}")
    logger.debug(f"DEBUG: conn_params.u8t0:{conn_params.u8t0}, u8t1:{conn_params.u8t1}, u8t2:{conn_params.u8t2}, u16t3:{conn_params.u16t3}")
    
    try:
        logger.debug(f"DEBUG: conn_params.eState: {conn_params.eState}")
    except Exception as e:
        logger.debug(f"DEBUG: Error reading eState: {e}")
    
    logger.debug(f"DEBUG: conn_params.u8TotalNumberofStations: {conn_params.u8TotalNumberofStations}")
    logger.debug(f"DEBUG: conn_params.u32GeneralInterrogationInterval: {conn_params.u32GeneralInterrogationInterval}")
    
    try:
        logger.debug(f"DEBUG: conn_params.eCOTsize: {conn_params.eCOTsize}")
    except Exception as e:
        logger.debug(f"DEBUG: Error reading eCOTsize: {e}")
    
    logger.debug(f"DEBUG: conn_params.u16NoofObject: {conn_params.u16NoofObject}")
    logger.debug(f"DEBUG: conn_params.psIEC104Objects is None?: {conn_params.psIEC104Objects is None}")
    logger.debug(f"DEBUG: bool(conn_params.psIEC104Objects): {bool(conn_params.psIEC104Objects)}")
    try:
        logger.debug(f"DEBUG: conn_params.psIEC104Objects address: {ctypes.addressof(conn_params.psIEC104Objects) if conn_params.psIEC104Objects else 'NULL'}")
    except Exception as e:
        logger.debug(f"DEBUG: Error getting psIEC104Objects address: {e}")
    try:
        logger.debug(f"DEBUG: conn_params.psIEC104Objects value: {conn_params.psIEC104Objects}")
    except Exception as e:
        logger.debug(f"DEBUG: Error getting psIEC104Objects value: {e}")
    
    logger.debug(f"DEBUG: conn_params.au16CommonAddress[0]: {conn_params.au16CommonAddress[0]}")
    logger.debug(f"DEBUG: conn_params.u8OrginatorAddress: {conn_params.u8OrginatorAddress}")
    logger.debug(f"DEBUG: conn_params.u32CommandTimeout: {conn_params.u32CommandTimeout}")
    logger.debug(f"DEBUG: conn_params.u32FileTransferTimeout: {conn_params.u32FileTransferTimeout}")
    logger.debug(f"DEBUG: conn_params.bCommandResponseActtermUsed: {conn_params.bCommandResponseActtermUsed}")
    logger.debug(f"DEBUG: conn_params.bEnablefileftransfer: {conn_params.bEnablefileftransfer}")
    logger.debug(f"DEBUG: conn_params.bUpdateCallbackCheckTimestamp: {conn_params.bUpdateCallbackCheckTimestamp}")
    
    try:
        file_path_bytes = bytes(conn_params.ai8FileTransferDirPath[:10])
        logger.debug(f"DEBUG: conn_params.ai8FileTransferDirPath first 10 bytes: {file_path_bytes}")
    except Exception as e:
        logger.debug(f"DEBUG: Error reading ai8FileTransferDirPath: {e}")
    
    logger.debug("DEBUG: ---- End of sConf dump ----")
    
    logger.info(f"Loading configuration: Server={scada_ip}:{scada_port}, CASDU={casdu_addr}")
    
    logger.info("DEBUG: About to call IEC104LoadConfiguration.")
    ret_load = iec104_lib.IEC104LoadConfiguration(g_client_handle, ctypes.byref(sConf), ctypes.byref(tErrVal))
    logger.info(f"DEBUG: IEC104LoadConfiguration returned {ret_load}. ErrVal: {tErrVal.value}")
    ...
    logger.info("DEBUG: About to call IEC104Start.")
    ret_start = iec104_lib.IEC104Start(g_client_handle, ctypes.byref(tErrVal))
    logger.info(f"DEBUG: IEC104Start returned {ret_start}. ErrVal: {tErrVal.value}")
    ...
    time.sleep(3)  # 增加等待时间给回调更多时间
    if g_connection_successful: 
        logger.info("SCADA Connection successful (callback confirmed).")
        return True
    else: 
        logger.error("SCADA Connection FAILED (callback not confirmed after timeout).")
        disconnect_scada()
        return False
